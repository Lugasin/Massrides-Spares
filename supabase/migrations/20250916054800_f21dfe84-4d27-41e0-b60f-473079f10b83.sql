-- Simple idempotent migration to populate spare_parts table with mock data
-- This version avoids foreign key issues by using null vendor_id initially
...
BEGIN
  ...
  -- Insert spare parts if they don't exist (using null vendor_id for now)
  ...
  IF NOT EXISTS (SELECT 1 FROM public.spare_parts WHERE part_number = '87540915') THEN
    INSERT INTO public.spare_parts (
      ...
      '18 months', null, null, true,
      ARRAY['electrical', 'alternator', 'charging'],
      electrical_cat_id, null, 'Case IH'
    );
  END IF;

  -- Add more parts with different categories
  IF NOT EXISTS (SELECT 1 FROM public.spare_parts WHERE part_number = 'HYD-PUMP-002') THEN
    INSERT INTO public.spare_parts (
      ...
      'HYD-PUMP-002', 285.00, 'new', 'in_stock', 35,
      ARRAY['/src/assets/Newtractor7.png', '/src/assets/Newtractor8.png'],
      '{"bore": "3 inches", "stroke": "12 inches", "rodDiameter": "1.5 inches", "workingPressure": "2500 PSI"}',
  ...
  IF NOT EXISTS (SELECT 1 FROM public.spare_parts WHERE part_number = 'FUEL-FILTER-001') THEN
    INSERT INTO public.spare_parts (
      ...
      'Fuel filter with water separator for diesel engines. Removes water and contaminants from fuel.',
      'FUEL-FILTER-001', 125.00, 'new', 'in_stock', 45,
      ARRAY['/src/assets/Newtractor9.png', '/src/assets/Newtractor10.png'],
      '{"filterType": "Fuel/Water Separator", "micronRating": "10 micron", "waterCapacity": "6 oz", "flowRate": "45 GPH"}',
