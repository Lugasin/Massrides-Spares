-- Vesicash Transaction Reconciliation Migration (V2)
...
-- Fixed: references 'user_profiles' instead of 'profiles'.
...
INSERT INTO products (id, sku, title, description, price, is_active, stock_quantity, brand, condition, availability_status, category_id, vendor_id)
VALUES (9999, 'VSH-HISTORICAL', 'Historical Order Correction', 'Reconciled from Vesicash dashboard', 0, false, 0, 'System', 'used', 'out_of_stock', (SELECT id FROM categories LIMIT 1), (SELECT id FROM user_profiles WHERE role = 'super_admin' LIMIT 1))
...
BEGIN
    FOREACH entry SLICE 1 IN ARRAY t_reconciliation_data LOOP
        -- A. Find user ID if exists (Using user_profiles instead of profiles)
        SELECT id INTO v_user_id FROM user_profiles WHERE email = entry[1] LIMIT 1;
