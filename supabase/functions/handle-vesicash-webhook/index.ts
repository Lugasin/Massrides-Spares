...
import { loadVesicashConfig, normaliseVesicashStatus, type PaymentStatus, settlePayment, resolveNextOrderStatus } from "../_shared/vesicash.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const nextPaymentStatus = normaliseVesicashStatus(String(data.status ?? ""), eventType);
    ...
    if (!isDuplicate) {
      const { platformFee, vendorEarning, nextOrderStatus } = await settlePayment(
        supabaseAdmin,
        payment,
        order as any,
        nextPaymentStatus,
      ...
      // Re-log the settled state for audit
      await supabaseAdmin.from("financial_audit_logs").insert({
        event_type: `payment_${nextPaymentStatus}`,
        entity_type: "payment",
        entity_id: String(payment.id),
        amount: Number(data.amount ?? order.total_amount ?? 0) || null,
        metadata: {
