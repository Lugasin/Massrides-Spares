...
create policy "Spare parts are public readable" on public.spare_parts for select using (true);
...
create policy "Compatibility public read" on public.spare_part_compatibility for select using (true);
...
create policy "Users can manage own carts" on public.user_carts
for all to authenticated
...
create policy "Users manage items in their carts" on public.cart_items
for all to authenticated
...
create policy "Guests can manage guest carts" on public.guest_carts
...
create policy "Guests can manage guest cart items" on public.guest_cart_items
for all to anon using (true) with check (true);
...
create policy "Users manage items in their orders" on public.order_items
for all to authenticated
...
create policy "Participants can access conversations" on public.conversations
for all to authenticated
...
create policy "Participants can manage messages" on public.messages
for all to authenticated
...
create policy "Admins read tj logs" on public.tj_transaction_logs
for select to authenticated
using (public.has_role(auth.uid(),'admin') or public.has_role(auth.uid(),'super_admin'));
